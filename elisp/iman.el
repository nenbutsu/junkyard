...
(defmacro iman-minibuffer-prompt-end ()
  (if (fboundp 'minibuffer-prompt-end)
      '(minibuffer-prompt-end)
    '(point-min)))

(defsubst iman-minibuffer-string ()
  (buffer-substring (iman-minibuffer-prompt-end) (point-max)))
...
possible completions like ls(1), lsattr(1), lsdic(1), and lsearch(3)."
  (interactive)
  (let* ((buf-str (iman-minibuffer-string))
     ...
     ((string= suffix "")
      (let ((strings '(" " "(" "-")))
        (unless (catch 'inserted
                  (while strings
                    (when (try-completion (concat (iman-minibuffer-string)
                                                  (car strings))
                                          minibuffer-completion-table
                                          minibuffer-completion-predicate)
                      (goto-char (point-max))
