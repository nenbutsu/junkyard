...
;;  I mainly wrote this package to learn the differences in the menu APIs
;;  between GNU Emacs and XEmacs, so I'm afraid there's no earth-shattering
;;  features that tmenu.el can be proud of.
;;
;;  tmm.el
;;   * is a standard package bundled with GNU Emacs.
;;   * is lightweight.
;;   * 
;;
;;  tmenu.el
;;   * runs on GNU Emacs and XEmacs.
;;   * allows the user to move around the menu hierarchy.
...
;;    the Emacsen (GNU Emacs or XEmacs).
;;
...
;;   functions. See the `tmenu-item core functions' section.
;;
...
;;
;; REAL-BINDING: COMMAND | TMENU | TMENU-LAMBDA | nil
;;
...
;; * Stromlund suggested key bindings customization and reported a history
;;   variable bug.
...
;;   don't work.
;; * The current user interface uses Emacs's completion facility.
...
;;
;;  * Fixed a minibuffer history pollution bug reported by Stromlund.
;;  * Fixed a bug where tmenu-merge-keymaps didn't merge minor mode keymaps.

;; Version 0.23
;;  * Fixed a bug where tmenu-merge-keymaps destroyed some keymap entries
;;    when the local keymap tried to undefine some of the global-map.
...
If you feel that tmenu is slow, turning this off might speed it up."
  :type 'boolean
  :group 'tmenu)
...
Since XEmacs complains that the value of `last-buffer-undo-list' is void
when evaluating :active property for some menu items, we have to ignore any
errors."
  (condition-case nil
      (eval form)
    (error nil)))
...
(defun tmenu-item-create (name real-binding &optional alist)
  ...
  (let ((item (nconc (list 'tmenu-item name real-binding)
                     (copy-alist alist))))
...
(defun tmenu-menu-read-from-minibuffer-1 (item)
  ...
  (let* ((name (tmenu-item-name item))
         ...
         (tmenu-history (mapcar 'car table))
    ...
    (cdr (assoc
          (completing-read (concat name ": ") table nil t nil 'tmenu-history)
          table))))
...
(defun tmenu-compute-menubar-keymap (&optional fake-function-keyseq)
  ...
  (let ((key (or fake-function-keyseq [menu-bar])))
    ...
    (nconc (tmenu-merge-keymaps
            (delq nil (apply 'list
                             (global-key-binding key)
                             (local-key-binding key)
                             (tmenu-compute-minior-mode-keymaps key)))
            (when (equal key [menu-bar])
              menu-bar-final-items))
           (list tmenu-menubar-prompt))))

(defun tmenu-compute-minior-mode-keymaps (&optional fake-function-keyseq)
  (let ((key (or fake-function-keyseq [menu-bar])))
    (delq nil
          (mapcar #'(lambda (entry)
