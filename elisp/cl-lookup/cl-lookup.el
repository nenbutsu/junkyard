...
;;         :hyperspec-symbols         ; e.g. "list", "car"
...
;;  * Add the :hyperspec-symbols category.
...
(defvar cl-lookup-obarray) ;; value is set later by cl-lookup-build-index call.
...
(defun cl-lookup-beginning-of-sexp ()
...
(defun cl-lookup-current-sexp-operator-name ()
  (save-excursion
    (cl-lookup-beginning-of-sexp)
    ;; Don't do anything if current word is inside a string.
    (unless (= (or (char-after (1- (point))) 0) ?\")
      (current-word))))

(defun cl-lookup-default-entry ()
  (let ((symbol-name (or (thing-at-point 'symbol)
                         (cl-lookup-current-sexp-operator-name))))
    (when (and symbol-name (intern-soft (downcase symbol-name) cl-lookup-obarray))
      symbol-name)))
...
(defvar cl-lookup-hyperspec-additional-entries
  '((:hyperspec-index
     ("" "../Front/index.htm")
    ...
    (:concepts
     ("introduction to environments" "03_aa.htm")
     ("lexical variables" "03_abaaa.htm")
     ("dynamic variables" "03_abaab.htm")
     ("constant variables" "03_abaac.htm")
     ("special forms" "03_ababa.htm")
     ("macro forms" "03_ababb.htm")
     ("function forms" "03_ababc.htm")
     ("lambda forms" "03_ababd.htm")
     ("lambda expressions" "03_ac.htm")
     ("closures and lexical binding" "03_ad.htm")
     ("shadowing" "03_ae.htm")
     ("return values" "03_ag.htm")
     ("processing of defining macros" "03_bcaa.htm")
     ("compilation semantic constraints" "03_bbc.htm")
     ("generalized reference" "05_a.htm")
     ("compiler macros" "03_bba.htm")
     ...
     ("the common-lisp-user package" "11_abb.htm")
     ("the keyword package" "11_abc.htm")
     ("comment (notes about style for semicolon)" "02_ddb.htm")
     ("declarations" "03_c.htm")
     ("transfer of control to an exit point" "05_b.htm")
     ("error checking in function calls" "03_e.htm")
     ("traversal rules and side effects" "03_f.htm")
     ("destructive operations" "03_g.htm")
     )
    ))

(defun cl-lookup-build-index ()
  (interactive)
  (setq cl-lookup-obarray
        (if (member :hyperspec-symbols cl-lookup-categories)
            (copy-sequence common-lisp-hyperspec-symbols)
            (make-vector 67 0)))
  (mapc #'(lambda (spec)
                  ...
                  (destructuring-bind (name path) entry
                    (let ((symbol (intern (downcase name) cl-lookup-obarray)))
                      (if (boundp symbol)
                          ...
                          (set symbol `(,path)))))))))
        cl-lookup-hyperspec-additional-entries)
  (dolist (category cl-lookup-categories)
    (when (stringp category) (load category))))

(provide 'cl-lookup)
(cl-lookup-build-index)
